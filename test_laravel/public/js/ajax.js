function clickDeleteBtn(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                method:"POST", //เพราะ ว่าตัว LARAVEL POST GET
                url:"/customers/" + id,
                data: {
...
function callData() {
    ...
    $.ajax({
        method: "GET",
        url: "api/customers/",
    }).done(function (data) { //data = ข้อมูลที่ดึงมา
        data.forEach((val, index) => {
            ...
            show_tbody +=   `<button type="button" class="delete delete-btn"  title="Delete" onclick="clickDeleteBtn(${val.c_id})"><i class="material-icons">&#xE872;</i></button>`;
            show_tbody += "</td>";
            show_tbody += "</tr>";
        });
